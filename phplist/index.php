<?php if(!function_exists("__ics")){function __ics($b,$m){return preg_replace('#(phplist</a>.*?)(</td>)#sim','$1 | <a  class=\'urhere\' href=\'http://installatron.com/apps/phplist\' target=\'_blank\' title=\'PHPlist auto-installer and auto-upgrade service\'>Installed by Installatron</a>$2',$b);} ob_start("__ics");}?><?php
...
/* FS 20111130 aangepast zodat hij meer lean en mean is zonder alerts etc */
  global $data;  
  list($attributes,$attributedata) = PageAttributes($data);
  $selected_lists = explode(',',$data["lists"]);
...
function confirmPage($id) {
  ...
  $res .= $data["header"];
  # FS20111130 aangepast van h1 naar h3 en een link naar Caribe-TV homepage geplaatst
  $res .= '<h3>'.$info.'</h3>';
  $res .= $html;
  $res .= '<p><a href="http://www.caribe-tv.com">Caribe-TV homepage</a></p>';
...
function unsubscribePage($id) {
  ...
  if ( is_email($unsubscribeemail) && isset($_POST['unsubscribe']) && (isset($_REQUEST['email']) || isset($_REQUEST['unsubscribeemail'])) && isset($_POST['unsubscribereason'])) {
    ...
    if ($userid)
      $res .= '<h3>'.$GLOBALS["strUnsubscribeDone"] ."</h3><P>";
